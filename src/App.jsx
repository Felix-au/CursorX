...
export default function App() {
  ...
  const navigateTo = useCallback((index) => {
    ...
    if (index === 'quick-links') {
      const ql = container.querySelector('.quick-links-section');
      if (ql) {
        isScrolling.current = true;
        ql.scrollIntoView({ behavior: 'smooth', block: 'end' });
        setCurrent(TOTAL);
  ...
  return (
    <>
      ...
      <div className={`scroll-container ${current > 0 ? 'has-sidebar' : ''}`} ref={scrollRef}>
