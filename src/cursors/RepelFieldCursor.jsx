...
export default function RepelFieldCursor({ containerRef, config }) {
  ...
  useEffect(() => {
    ...
    ring.style.cssText = `
      ...
      border:2px solid ${config.color ?? '#fc5cb8'};
      transform:translate(-50%,-50%);
      box-shadow: 0 0 12px ${config.color ?? '#fc5cb8'}55;
    ...
    dot.style.cssText = `
      ...
      background:${config.color ?? '#fc5cb8'};
      transform:translate(-50%,-50%);
    `;
    container.appendChild(ring);
    container.appendChild(dot);

    const targets = [...container.querySelectorAll('[data-repel]')];
    ...
    const loop = () => {
      ...
      const strength = cfg.strength ?? 20000;
      const radius   = cfg.radius   ?? 190;
      const damping  = cfg.damping  ?? 0.80;
      const color    = cfg.color    ?? '#fc5cb8';
